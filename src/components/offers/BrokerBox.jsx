...
export default function BrokerBox({ offer, isTopPick = false }) {
  ...
  return (
    ...
    >
      ...
      <Link
        to={`/broker/${slug ?? id}`}
        aria-label={`Read the full ${name} review`}
        className="absolute right-6 top-6 inline-flex rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent"
      >
        <ScoreBadge
          score={computedScore}
          className="shadow-lg shadow-emerald-500/20 transition hover:shadow-emerald-500/30"
        />
      </Link>

      <div className="flex flex-col gap-4 pr-0 sm:pr-6">
